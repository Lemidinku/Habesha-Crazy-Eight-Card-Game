        ...
        {room.players.map((p) => (
          <div
            ...
            className={[
              ...
              p.handCount === 1
                ? 'ring-2 ring-crimson shadow-[0_0_10px_var(--color-crimson)] animate-pulse motion-reduce:animate-none'
                : '',
            ].join(' ')}
          >
            <div className="font-medium text-card">
              {p.displayName}
              {p.handCount === 1 && <span className="text-crimson text-xs ml-1.5">{t('gameTable.lastCard')}</span>}
            </div>
